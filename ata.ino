...
const unsigned long TEMPO_MAXIMO_GIRO_NORTE_MS = 2000;
...
const int TOLERANCIA_ANGULO_SUL_PRECISA = 5;
...
const int DRIBBLER_GIRO_NORTE_LONGE_US = 1210;
const int DRIBBLER_GIRO_NORTE_PERTO_US = 1000; // velocidade do dribbler durante o chute

// ===== VELOCIDADES DA LOGICA COM BOLA =====
const int VELOCIDADE_GIRO_SUL_COM_BOLA = 100;
const int VELOCIDADE_GIRO_SUL_FINAL = 55;
const int VELOCIDADE_GIRO_NORTE_LENTO = 170;
const int VELOCIDADE_GIRO_NORTE_RAPIDO = 230;
const int ANGULO_RESTANTE_PARA_GIRO_NORTE_RAPIDO = 75;
const int ANGULO_RESTANTE_PARA_DRIBBLER_GIRO_NORTE_PERTO = 35; // andulo restante para o dribbler
...
const int LIMIAR_LINHA = 100; // alterar para sensor de linha na quadra
const float BNO_ZONA_MORTA_GRAUS = 0.5;
const bool GIRO_SUL_PARA_ESQUERDA = true;
const bool GIRO_NORTE_PARA_ESQUERDA = false;
const bool MOSTRAR_VALORES_LDR = true;
const unsigned long INTERVALO_PRINT_LDR_MS = 500;

unsigned long ultimoPrintLdr = 0;
...
void mostrar_valores_ldr() {
  if (!DEBUG_SERIAL_ATIVO || !MOSTRAR_VALORES_LDR) {
    return;
  }

  if (millis() - ultimoPrintLdr < INTERVALO_PRINT_LDR_MS) {
    return;
  }

  ultimoPrintLdr = millis();

  // Atualiza os valores dos 16 sensores
  lerLDR();

  Serial.println("========= SENSORES DE LINHA =========");

  for (int i = 0; i < 16; i++) {
    bool detectouLinha;

    if (LINHA_VALOR_MENOR_QUE_QUADRA) {
      detectouLinha = ldr[i] < LIMIAR_LINHA;
    } else {
      detectouLinha = ldr[i] > LIMIAR_LINHA;
    }
    ...
    Serial.print(ldr[i]);
    Serial.print(" | Limiar: ");
    Serial.print(LIMIAR_LINHA);
    Serial.print(" | ");

    if (detectouLinha) {
      Serial.println("LINHA");
    } else {
      Serial.println("QUADRA");
    }
  }
...
bool escolher_lado_maior_ultrassonico() {
  if (ultimaDistanciaDireitaCm <= 0 && ultimaDistanciaEsquerdaCm <= 0) {
    debug_println("Sem decisao apos 5 leituras validas. Usando direita por padrao.");
    return true;
  }
  ...
  if (ultimaDistanciaDireitaCm == ultimaDistanciaEsquerdaCm) {
    debug_println("Laterais empatadas. Usando direita por padrao.");
    return true;
  }
...
bool escolher_giro_sul_para_esquerda_por_ultrassonico() {
  debug_println("Giro ate o sul fixo para a direita.");
  return false;
}
...
bool olhar_para_sul_com_bola_lento() {
  const bool girarParaEsquerda = false;

  debug_println("Sentido fixo para olhar para o sul: direita");
...
bool olhar_para_norte_duas_etapas_com_bola() {
  ...
  while (true) {
    ...
    ajustar_dribbler_giro_norte(erroAbs);
    ...
    if (millis() - inicio > TEMPO_MAXIMO_GIRO_NORTE_MS) {
      stop_robot();
      return false;
    }
...
bool girar_ate_angulo_com_bola_fixo_sentido(float alvo, int velocidade, unsigned long tempoMaximo, bool paraEsquerda) {
  unsigned long inicio = millis();
  float erroAnterior = 999;
  bool jaChegouPerto = false;
  ...
  while (true) {
    ...
    bool passouDoAlvo = false;
    if (jaChegouPerto && erroAnterior != 999) {
      passouDoAlvo = (paraEsquerda && erroAnterior > 0 && erro < 0) || (!paraEsquerda && erroAnterior < 0 && erro > 0);
    }

    if (erroAbs <= TOLERANCIA_ANGULO_SUL_PRECISA || passouDoAlvo) {
      debug_print("BNO alvo ok: ");
      debug_println(alvo);
      debug_print("Erro final sul: ");
      debug_println(erro);
      stop_robot();
      return true;
    }
    ...
    if (erroAbs <= 35) {
      velocidadeAtual = VELOCIDADE_GIRO_SUL_FINAL;
    }
    ...
    if (millis() - inicio > tempoMaximo) {
      debug_println("Tempo maximo para alinhar no sul. Parando e seguindo mesmo assim.");
      stop_robot();
      return true;
    }
...
void dribbler_velocidade_us(int microseconds) {
  if (dribblerMicroAtual == microseconds) {
    return;
  }

  esc.writeMicroseconds(microseconds);
  dribblerMicroAtual = microseconds;
  dribblerLigado = microseconds != DRIBBLER_PARADO_US;
}

void ajustar_dribbler_giro_norte(float erroAbs) {
  if (erroAbs <= ANGULO_RESTANTE_PARA_DRIBBLER_GIRO_NORTE_PERTO) {
    dribbler_velocidade_us(DRIBBLER_GIRO_NORTE_PERTO_US);
  } else {
    dribbler_velocidade_us(DRIBBLER_GIRO_NORTE_LONGE_US);
  }
}

void dribbler_velocidade_procura() {
  dribbler_velocidade_us(DRIBBLER_VELOCIDADE_PROCURA_US);
}

void dribbler_parado() {
  dribbler_velocidade_us(DRIBBLER_PARADO_US);
}

void dribbler_velocidade_com_bola() {
  dribbler_velocidade_us(DRIBBLER_VELOCIDADE_COM_BOLA_US);
}

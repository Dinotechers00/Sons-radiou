...
unsigned long inicioMovimentoLinha = 0;
bool movimentoLinhaAtivo = false;
int direcaoMovimentoLinha = 0;
// 1 = ré
// 2 = frente

const unsigned long TEMPO_MOVIMENTO_LINHA = 400;  
...
void controlar_goleiro() {
  ...
  // Linha traseira tem prioridade.
  // Ao detectar a linha preta, o goleiro dá ré por 0,5 segundo.
  if (linhaTraseira) {
  ...
  // Linha frontal tem prioridade depois da traseira.
  // Ao detectar a linha preta, o goleiro avança por 0,5 segundo.
  ...
  // Sem bola, ou com a bola ao centro, ele para.
  if (ballDirecao == 0 || ballDirecao == 5) {

    stop_robot();

    acaoAtual = ballDirecao == 0
                ? "robo parado: sem bola"
                : "robo parado: bola ao centro";
